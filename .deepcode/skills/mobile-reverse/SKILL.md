...
name: mobile-reverse
description: >
  用于授权的 Android/iOS 应用逆向与安全测试，覆盖 APK/IPA 分析、运行时插桩、SSL pinning、平台防护检查等。
version: 1.0.0
...
| 工具 | 平台 | 用途 |
|------|:--:|------|
| JADX-GUI | A | Java 反编译（本地 v1.5.6：`tools/jadx/`，CLI 启动器 `bash tools/pentest/bin/jadx.sh`） |
| apktool | A | APK 解包/重建（本地 v3.0.3：`tools/apktool/apktool_3.0.3.jar`，启动器 `bash tools/pentest/bin/apktool.sh`） |
| Ghidra | A+I | 多架构反编译 |
| Hopper | I | iOS 专用反汇编 |
| Frida | A+I | 动态插桩（本地 v17.16.4 pip 安装，启动器 `bash tools/pentest/bin/frida.sh`） |
| Objection | A+I | Frida REPL 增强（本地 v1.12.5 pip 安装，启动器 `bash tools/pentest/bin/objection.sh`） |
