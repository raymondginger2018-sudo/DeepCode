...
def lookup_conditions(po):
    """按 PO 号代号查默认商业条款，找不到返回空字符串。"""
    code = supplier_code(po)
    if code and code in _CODE_MAP:
        return _CODE_MAP[code].get('conditions', '')
    return ''

def lookup_warranty(po):
    """按 PO 号代号查默认质保条款，找不到返回空字符串。"""
    code = supplier_code(po)
    if code and code in _CODE_MAP:
        return _CODE_MAP[code].get('warranty', '')
    return ''
...
def build_row(info, template_row):
    ...
    return {
        ...
        3: info.get('conditions') or lookup_conditions(po),  # C COMMERCIAL CONDITIONS
        4: info.get('warranty') or lookup_warranty(po),      # D PRODUCT WARRANTY
        ...
        15: info.get('container') or (_to_float(info.get('cbm')) / 65 if info.get('cbm') else None),  # O CONTAINER QTY = CBM/65
