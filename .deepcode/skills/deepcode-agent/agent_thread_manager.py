...
class AgentThread:
    ...
    # ── 运行时编导 (Hermes steer 模式) ──
    pending_steer: str = ""        # 父线程排队的中途纠偏文本
    # ── 所有权标记 (Hermes 双层判定) ──
    owner_pid: int = 0             # Tier 1: 派生者进程 PID (0=未知)
...
class AgentThreadManager:
    ...
    def _ensure_schema(self) -> None:
        ...
        db.executescript("""
            CREATE TABLE IF NOT EXISTS agent_threads (
                ...
                pending_steer   TEXT NOT NULL DEFAULT '',
        ...
        db.execute(
            """INSERT INTO agent_threads
               (id, goal, agent_type, status, context_json,
                ...
                token_budget, max_calls, sandbox, pending_steer, owner_pid)
               VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
            (
                ...
                thread.pending_steer, thread.owner_pid,
            ),
        )
    ...
    # ── 运行时编导 (Hermes steer 模式 + 所有权双层判定) ──

    MAX_OWNERSHIP_HOPS = 8  # 对标 Hermes _is_descendant_of max_hops

    def is_descendant_of(self, descendant_id: str, ancestor_id: str) -> bool:
        """Tier 2 持久谱系判定: descendant 是否在 ancestor 的 spawn 树内。

        沿 parent 链最多走 MAX_OWNERSHIP_HOPS 跳 (对标 Hermes weakref 链)。
        同线程视为 true (自己控制自己, 与 Hermes _owns_subagent_record 一致)。
        """
        ...
        if descendant_id == ancestor_id:
            return True
        cur = descendant_id
        for _ in range(self.MAX_OWNERSHIP_HOPS):
            parent = self.get_parent_thread(cur)
            if parent is None:
                return False
    ...
    def owns_thread(self, owner_thread_id: str, target_thread_id: str) -> Tuple[bool, str]:
        """所有权双层判定 — Tier 2 持久谱系 + 存在性检查。

        返回 (ok, reason)。owner 必须是 target 本身或 target 的祖先
        (spawn 树上层才有权 steer/stop/delete)。
        """
        target = self.get(target_thread_id)
        if target is None:
            return False, f"target thread not found: {target_thread_id}"
        if not owner_thread_id:
            return False, "owner_thread_id required"
        owner = self.get(owner_thread_id)
        if owner is None:
            return False, f"owner thread not found: {owner_thread_id}"
        if not self.is_descendant_of(target_thread_id, owner_thread_id):
            return False, (
                f"ownership denied: {owner_thread_id} is not an ancestor of {target_thread_id}"
            )
        return True, ""
    ...
    def steer_thread(
        ...
        """给子线程排队一条 steer 文本 — 不打断工具，追加到 pending_steer。

        对标 Hermes `agent.steer(text)`: 文本在子 Agent 当前工具批次结束后
        作为新的 user 消息注入 (由 auto_spawn 执行器在下一轮读取)。
        """
        if not text or not text.strip():
            return {"ok": False, "reason": "steer text empty"}
        ok, reason = self.owns_thread(owner_thread_id, thread_id)
        if not ok:
            return {"ok": False, "reason": reason}
        thread = self.get(thread_id)
        existing = thread.pending_steer if thread else ""
        new_steer = (existing + "\n" + text.strip()) if existing else text.strip()
        db = self._connect()
        db.execute(
            "UPDATE agent_threads SET pending_steer = ?, updated_at = ? WHERE id = ?",
            (new_steer, datetime.now(timezone.utc).isoformat(), thread_id),
        )
        db.commit()
        self._log(thread_id, LogLevel.INFO,
                  f"Steer queued ({len(text.strip())} chars) from {owner_thread_id}",
                  {"owner_thread_id": owner_thread_id})
        return {"ok": True, "queued": True, "thread_id": thread_id,
                "pending_len": len(new_steer)}

    def get_pending_steer(self, thread_id: str) -> str:
        """读取排队中的 steer 文本 (供 auto_spawn 执行器轮询)。"""
        thread = self.get(thread_id)
        return thread.pending_steer if thread else ""

    def drain_steer(self, thread_id: str) -> dict:
        """原子读取并清空 pending_steer — 执行器在注入下一轮 prompt 前调用。"""
        thread = self.get(thread_id)
        if thread is None:
            return {"ok": False, "reason": "thread not found"}
        text = thread.pending_steer
        if text:
            db = self._connect()
            db.execute(
                "UPDATE agent_threads SET pending_steer = '', updated_at = ? WHERE id = ?",
            ...
            db.commit()
        return {"ok": True, "text": text, "drained": bool(text)}

    def clear_steer(self, thread_id: str, owner_thread_id: str) -> dict:
        """清空排队中的 steer (所有权校验)。"""
        ok, reason = self.owns_thread(owner_thread_id, thread_id)
        if not ok:
            return {"ok": False, "reason": reason}
        ...
        db.commit()
        return {"ok": True, "cleared": True, "thread_id": thread_id}
    ...
    def stop_thread_cascade(self, thread_id: str,
                           caller_thread_id: Optional[str] = None) -> int:
        """级联停止线程及其所有子线程。

        若提供 caller_thread_id，先做所有权校验（caller 必须是 thread 的祖先）。
        """
        if caller_thread_id:
            ok, reason = self.owns_thread(caller_thread_id, thread_id)
            if not ok:
                self._log(thread_id, LogLevel.WARN,
                          f"Stop denied: {reason}", {})
                return 0
        db = self._connect()
        children = self.get_child_threads(thread_id)
        stopped = 0
        for child in children:
            stopped += self.stop_thread_cascade(child.id, caller_thread_id)

        self.update_status(thread_id, ThreadStatus.STOPPED)
        stopped += 1
        return stopped

    def delete_thread_cascade(self, thread_id: str,
                             caller_thread_id: Optional[str] = None) -> int:
        """级联删除线程及其所有子线程、动态工具、日志。

        若提供 caller_thread_id，先做所有权校验。
        """
        if caller_thread_id:
            ok, reason = self.owns_thread(caller_thread_id, thread_id)
            if not ok:
                self._log(thread_id, LogLevel.WARN,
                          f"Delete denied: {reason}", {})
                return 0
        db = self._connect()
        children = self.get_child_threads(thread_id)
        deleted = 0
        for child in children:
            deleted += self.delete_thread_cascade(child.id, caller_thread_id)
    ...
    def _row_to_thread(self, row: sqlite3.Row) -> AgentThread:
        ...
        return AgentThread(
            ...
            pending_steer=_get("pending_steer", "") or "",
            owner_pid=int(_get("owner_pid", 0) or 0),
        )
...
def _launch_auto_executor(
    ...
    """Spawn 后立即在后台实际执行 goal，避免线程永远停留在 created。

    执行通道: 独立子进程 `auto_spawn.py exec <payload>` (headless 子 DeepCode)。
    子进程完成/失败后通过 AgentThreadManager.update_status() 更新线程状态
    (created → running → completed/failed)，与 auto_spawn.py 的 _execute_worker 闭环一致。
    该调用不阻塞 MCP 主循环。
    """
    ...
    try:
        ...
        payload = _json.dumps({
            "prompt": goal,
            "thread_id": thread_id,
            "scenario": "agent_thread_spawn",
            "agent_type": agent_type_str,
            "timeout": int(timeout or 600),
            "max_iterations": int(max_iterations or 20),
            "model": model or "deepseek-v4-flash",
        }, ensure_ascii=False)
        auto_spawn_py = Path(__file__).resolve().parent / "auto_spawn.py"
        subprocess.Popen(
            [sys.executable, str(auto_spawn_py), "exec", payload],
            cwd="F:/DEEPCODE",
        ...
        try:
            mgr.update_status(thread_id, ThreadStatus.FAILED, error=f"executor launch failed: {e}")
        except Exception:
            pass
...
async def run_mcp(db_path: str = ""):
    ...
    TOOLS = {
        ...
        "agent_thread_stop": {
            "description": "级联停止线程及其所有子线程（支持所有权校验：传 caller_thread_id 后仅祖先可停止）。",
            "inputSchema": {
                "type": "object",
                "properties": {
                    "thread_id": {"type": "string"},
                    "caller_thread_id": {"type": "string", "description": "调用者线程 ID（可选），用于所有权判定"},
                },
                "required": ["thread_id"],
            },
        },
        "agent_thread_delete": {
            "description": "级联删除线程及其子线程、动态工具、日志（支持所有权校验：传 caller_thread_id 后仅祖先可删除）。",
            "inputSchema": {
                "type": "object",
                "properties": {
                    "thread_id": {"type": "string"},
        ...
        "agent_thread_steer": {
            "description": "给子线程排队 steer 文本 — 不打断工具，追加到 pending_steer（由 auto_spawn 下一轮读取注入）。需提供 owner_thread_id（父或祖先线程）做所有权校验。",
            "inputSchema": {
                "type": "object",
                "properties": {
                    "thread_id": {"type": "string", "description": "目标子线程 ID"},
                    "text": {"type": "string", "description": "steer 文本内容"},
                    "owner_thread_id": {"type": "string", "description": "调用者线程 ID（父/祖先，用于所有权校验）"},
                },
                "required": ["thread_id", "text", "owner_thread_id"],
            },
        },
        "agent_thread_steer_drain": {
            "description": "原子读取并清空 pending_steer — 执行器在注入下一轮 prompt 前调用。返回当前排队文本及是否 drained。",
            "inputSchema": {
                "type": "object",
                "properties": {
                    "thread_id": {"type": "string"},
        ...
        "agent_thread_steer_clear": {
            "description": "清空排队中的 steer 文本（需要所有权校验）。",
            "inputSchema": {
                "type": "object",
                "properties": {
                    "thread_id": {"type": "string"},
                ...
                "required": ["thread_id", "owner_thread_id"],
            },
        },
    }
        ...
        if method == "tools/list":
            ...
            try:
                if name == "agent_thread_spawn":
                    ...
                    # 自动执行: 默认情况下 spawn 后立即在后台实际执行 goal,
                    # 避免线程永远停留在 created (对标 codex.exe SubagentStart 的立即执行语义)。
                    # 仅当 approval_required=True 或 sandbox 隔离模式下等待外部接管。
                    if not args.get("approval_required", False) and not args.get("sandbox", ""):
                        _launch_auto_executor(
                            mgr=mgr,
                            thread_id=thread.id,
                            goal=args["goal"],
                    ...
                    count = mgr.stop_thread_cascade(
                        args["thread_id"],
                        caller_thread_id=args.get("caller_thread_id"),
                    )
                    result = {"stopped": count, "thread_id": args["thread_id"]}

                elif name == "agent_thread_delete":
                    count = mgr.delete_thread_cascade(
                        args["thread_id"],
                        caller_thread_id=args.get("caller_thread_id"),
                    )
                ...
                elif name == "agent_thread_steer":
                    result = mgr.steer_thread(
                        args["thread_id"], args["text"],
                        owner_thread_id=args["owner_thread_id"],
                    )

                elif name == "agent_thread_steer_drain":
                    result = mgr.drain_steer(args["thread_id"])

                elif name == "agent_thread_steer_clear":
                    result = mgr.clear_steer(
                        args["thread_id"], args["owner_thread_id"],
                    )
...
def main():
    ...
    # steer (运行时编导)
    p = sub.add_parser("steer", help="给线程排队 steer 纠偏文本（不打断工具）")
    p.add_argument("thread_id")
    p.add_argument("text", help="steer 提示文本")
    p.add_argument("--owner", default=None, help="调用者线程 ID（可选，不传则不校验所有权）")

    p = sub.add_parser("steer-status", help="查看线程的 pending_steer 文本")
    p.add_argument("thread_id")

    p = sub.add_parser("steer-clear", help="清空线程的 pending_steer")
    p.add_argument("thread_id")
    p.add_argument("--owner", default=None, help="调用者线程 ID（可选）")
    ...
    if args.command == "spawn":
        ...
        r = mgr.steer_thread(args.thread_id, args.text,
                             owner_thread_id=args.owner or "")
        _print_json(r)

    elif args.command == "steer-status":
        text = mgr.get_pending_steer(args.thread_id)
        print(text if text else "(no pending steer)")

    elif args.command == "steer-clear":
        r = mgr.clear_steer(args.thread_id, owner_thread_id=args.owner or "")
        _print_json(r)
