...
def _build_handoff_summary(final_text: str, thread_id: str, cfg: Dict) -> str:
    """模式 3: 摘要按父上下文 headroom 预算化, 超限 spill 落盘并附续读指引。

    对标 Hermes tools/delegate_tool_results.py `_apply_summary_budget()` +
    `_trim_summary_with_footer()`。父元数据缺失时退化为静态上限（保持旧行为）。
    """
    ...
    try:
        ...
        from thread_results import (  # noqa: PLC0415
            MIN_SUMMARY_CHARS,
            load_parent_meta,
            parent_summary_char_budget,
            trim_summary_with_footer,
        )

        hf = cfg.get("handoff", {}) or {}
        static_cap = int(hf.get("summary_max_chars", 1500))

        # 动态预算: 读父线程元数据里的 context 用量
        cap = static_cap
        parent_meta: Dict = {}
        if thread_id:
            try:
                from agent_thread_manager import AgentThreadManager  # noqa: PLC0415
                mgr = AgentThreadManager()
                try:
                    t = mgr.get(thread_id)
                    parent = mgr.get_parent_thread(t.id) if t is not None else None
                    if parent is not None:
                        parent_meta = load_parent_meta(parent.metadata_json)
                finally:
                    mgr.close()
            except Exception:
                parent_meta = {}

        if parent_meta.get("context_length"):
            dynamic = parent_summary_char_budget(
                context_length=int(parent_meta.get("context_length") or 0),
                prompt_tokens=int(parent_meta.get("prompt_tokens") or 0),
                compressor_max_tokens=int(parent_meta.get("compressor_max_tokens") or 0),
                n_summaries=1,
            )
            # 动态预算只用来**放宽**上限, 静态配置仍是下限保护
            cap = max(static_cap, min(dynamic, 24_000))

        spill_dir = Path(hf.get("spill_dir", "")) if hf.get("spill_dir") else \
            WORKERS_ROOT / "summaries"
        trimmed = trim_summary_with_footer(
            text, cap=cap, label=thread_id or "handoff",
            spill_dir=spill_dir, spill=True,
        )
        return trimmed.text or text[:MIN_SUMMARY_CHARS]
    except Exception as e:
        log(f"[handoff] 摘要预算化失败, 退化为硬截断: {e}")
        return text[:1500]
...
def handoff_result(scenario: str, agent_type: str, thread_id: str, final_text: str,
    ...
    try:
        ...
        # 提取 final_text 摘要 (模式 3: 预算化 + spill)
        summary = _build_handoff_summary(final_text, thread_id, cfg)
        note = (
            f"---\n"
