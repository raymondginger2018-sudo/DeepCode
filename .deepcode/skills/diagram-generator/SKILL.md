...
description: >
  从自然语言、笔记、代码片段、schema、表格或已有图表源生成/优化/校验/渲染图表，用于流程图、泳道图、时序图、状态图、ER 图、类图、架构/C4 图、依赖图、甘特图、思维导图、用户旅程、桑基图、组织图、网络图等。默认支持 mermaid，graphviz dot 用于复杂布局，plantuml 用于 UML 工程图，需直接标记时输出 svg。
version: 1.0.0
