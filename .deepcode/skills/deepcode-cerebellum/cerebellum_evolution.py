...
def _init_evolution_db(db_path: Path = DEFAULT_DB) -> None:
    ...
    conn.executescript(
        ...
        -- Skill 版本 DAG (E3, 对标 OpenSpace SkillLineage):
        -- 每次 apply 落一条版本记录 (generation+1, lineage, trust, content 快照, parent_hash)
        CREATE TABLE IF NOT EXISTS skill_lineage_versions (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            skill_name TEXT NOT NULL,
            generation INTEGER NOT NULL,
            lineage TEXT NOT NULL DEFAULT 'derived',   -- imported / captured / derived / fixed
            trust TEXT NOT NULL DEFAULT 'provisional', -- provisional / trusted / rejected
        ...
        CREATE INDEX IF NOT EXISTS idx_lin_skill ON skill_lineage_versions(skill_name, generation);
        """
    )
    ...
    try:
        ...
        pass  # 列已存在 → 幂等跳过
    # 旧库兼容 (E3): skill_evolution_proposals 增加版本 DAG / 证据信任列
    for col, ddl in (
        ("content_hash", "ALTER TABLE skill_evolution_proposals ADD COLUMN content_hash TEXT DEFAULT ''"),
        ("parent_hash", "ALTER TABLE skill_evolution_proposals ADD COLUMN parent_hash TEXT DEFAULT ''"),
        ("lineage", "ALTER TABLE skill_evolution_proposals ADD COLUMN lineage TEXT DEFAULT 'imported'"),
        ("trust", "ALTER TABLE skill_evolution_proposals ADD COLUMN trust TEXT DEFAULT 'provisional'"),
    ):
        try:
            conn.execute(ddl)
            conn.commit()
...
def skill_evolution_propose(skill_name: str, db_path: Path = DEFAULT_DB,
    ...
    # E3 版本 DAG: 计算当前 SKILL.md 内容哈希 + 血缘 (有历史版本 → derived, 否则 imported)
    current_content = ""
    ...
    if md_path and md_path.exists():
        try:
            ...
            current_content = ""
    current_hash = hashlib.sha256(current_content.encode("utf-8")).hexdigest()[:16] if current_content else ""
    has_history = conn.execute(
        "SELECT COUNT(*) FROM skill_lineage_versions WHERE skill_name=?", (skill_name,)
    ).fetchone()[0] > 0
    lineage = "derived" if has_history else "imported"
    cur = conn.execute(
        "INSERT INTO skill_evolution_proposals "
        "(skill_name, signal_count, failure_pattern, suggested_change, status, "
        " content_hash, parent_hash, lineage, trust, created_at, updated_at) "
        "VALUES (?, ?, ?, ?, 'pending', ?, ?, ?, 'provisional', ?, ?)",
        (skill_name, cnt, pattern, suggestion, current_hash, current_hash,
         lineage, now, now),
    )
    conn.commit()
    conn.close()
...
def skill_evolution_list(status: Optional[str] = None, db_path: Path = DEFAULT_DB,
    ...
    if status:
        rows = conn.execute(
            "SELECT id, skill_name, signal_count, failure_pattern, suggested_change, status, "
            "lineage, trust, content_hash, parent_hash, created_at "
...
def skill_evolution_apply(proposal_id: int, db_path: Path = DEFAULT_DB) -> Dict:
    ...
    row = conn.execute(
        ...
        "parent_hash, lineage FROM skill_evolution_proposals WHERE id=?", (proposal_id,)
    ).fetchone()
    if not row:
        conn.close()
    ...
    try:
        ...
        with open(md_path, "a", encoding="utf-8") as fh:
            fh.write(section)
        # E3 证据门控: apply 为人工确认路径 → trust=trusted; 落版本 DAG 快照
        ...
        lineage = row["lineage"] or "derived"
        max_gen = conn.execute(
            "SELECT COALESCE(MAX(generation), 0) FROM skill_lineage_versions WHERE skill_name=?",
            (row["skill_name"],),
        ).fetchone()[0]
        conn.execute(
            "INSERT INTO skill_lineage_versions "
            "(skill_name, generation, lineage, trust, content, content_hash, parent_hash, proposal_id, created_at) "
            "VALUES (?, ?, ?, 'trusted', ?, ?, ?, ?, ?)",
            (row["skill_name"], max_gen + 1, lineage, new_content, new_hash,
             parent_hash, proposal_id, datetime.now().isoformat(timespec="seconds")),
        )
        conn.execute(
            "UPDATE skill_evolution_proposals SET status='applied', trust='trusted', "
            "content_hash=?, updated_at=? WHERE id=?",
            (new_hash, datetime.now().isoformat(timespec="seconds"), proposal_id),
        )
        conn.commit()
        conn.close()
        return {"ok": True, "proposal_id": proposal_id, "skill_name": row["skill_name"],
                "applied_to": str(md_path), "generation": max_gen + 1, "lineage": lineage,
                "trust": "trusted", "content_hash": new_hash}
    except Exception as e:
        conn.close()
...
def skill_evolution_reject(proposal_id: int, db_path: Path = DEFAULT_DB) -> Dict:
    ...
    conn = _connect(db_path)
    conn.execute(
        "UPDATE skill_evolution_proposals SET status='rejected', trust='rejected', updated_at=? WHERE id=?",
...
# ═══════════════════════════════════════════
# 3.5 Skill 版本 DAG + 证据信任查询 (E3, 对标 OpenSpace SkillLineage)
# ═══════════════════════════════════════════

def skill_lineage_history(skill_name: str, db_path: Path = DEFAULT_DB,
                          limit: int = 50) -> Dict:
    """查看某 skill 的版本血缘 (generation / lineage / trust / hash / 提案号)。"""
    _init_evolution_db(db_path)
    conn = _connect(db_path)
    rows = conn.execute(
        "SELECT generation, lineage, trust, content_hash, parent_hash, proposal_id, created_at "
        "FROM skill_lineage_versions WHERE skill_name=? ORDER BY generation DESC LIMIT ?",
        (skill_name, limit),
    ).fetchall()
    conn.close()
    return {"ok": True, "skill_name": skill_name, "count": len(rows),
            "versions": [dict(r) for r in rows]}


def skill_version_diff(skill_name: str, from_generation: int, to_generation: int,
                       db_path: Path = DEFAULT_DB) -> Dict:
    """两个版本的 content 快照 diff (difflib, 行级)。"""
    _init_evolution_db(db_path)
    conn = _connect(db_path)
    def _get(g: int):
        return conn.execute(
            "SELECT generation, content FROM skill_lineage_versions "
            "WHERE skill_name=? AND generation=?", (skill_name, g)
        ).fetchone()
    a, b = _get(from_generation), _get(to_generation)
    conn.close()
    if not a or not b:
        return {"ok": False, "error": f"版本不存在: {from_generation}→{to_generation}"}
    diff = list(difflib.unified_diff(
        (a["content"] or "").splitlines(keepends=True),
        (b["content"] or "").splitlines(keepends=True),
        fromfile=f"{skill_name}@v{from_generation}",
        tofile=f"{skill_name}@v{to_generation}",
        lineterm="",
    ))
    return {"ok": True, "skill_name": skill_name,
            "from": from_generation, "to": to_generation,
...
def main() -> int:
    ...
    p_lin = sub.add_parser("skill_lineage", help="查看 skill 版本血缘 (E3)")
    p_lin.add_argument("--skill", required=True)
    p_lin.add_argument("--limit", type=int, default=50)
    p_diff = sub.add_parser("skill_diff", help="两个版本 content 快照 diff (E3)")
    p_diff.add_argument("--skill", required=True)
    p_diff.add_argument("--from", dest="from_g", required=True, type=int)
    p_diff.add_argument("--to", dest="to_g", required=True, type=int)
    ...
    if args.cmd == "dreaming":
        ...
        print(json.dumps(skill_evolution_reject(args.proposal_id),
                         ensure_ascii=False, indent=2))
    elif args.cmd == "skill_lineage":
        print(json.dumps(skill_lineage_history(args.skill, limit=args.limit),
                         ensure_ascii=False, indent=2))
    elif args.cmd == "skill_diff":
        print(json.dumps(skill_version_diff(args.skill, args.from_g, args.to_g),
                         ensure_ascii=False, indent=2))
