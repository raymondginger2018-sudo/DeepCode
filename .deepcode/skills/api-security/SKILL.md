...
name: api-security
description: >
  用于对 REST、GraphQL、WebSocket、SOAP 等 API 做授权的安全评估，覆盖接口发现、认证、授权、限流、CI/CD 测试等场景。
version: 1.0.0
...
本地调用（已安装 v2.3.0）:
  bash tools/pentest/bin/jwt_tool.sh <JWT> -M pb          # 攻击模式菜单
  bash tools/pentest/bin/jwt_tool.sh <JWT> -C -d wordlist.txt   # 弱 HMAC 密钥爆破
  bash tools/pentest/bin/jwt_tool.sh <JWT> -T -pc exp     # 篡改声明测试
...
| jwt_tool | JWT 全面测试 | 本地已装 v2.3.0：`tools/pentest/jwt_tool/`，启动器 `tools/pentest/bin/jwt_tool.sh`（依赖 termcolor/cprint/pycryptodomex/requests/ratelimit 已装） |
