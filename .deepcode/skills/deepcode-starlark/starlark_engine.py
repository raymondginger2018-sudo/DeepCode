...
# ── 确定性 PRNG (源自 Cursor crepectl 逆向: splitmix64) ────────
# Starlark 要求确定性执行, 这里提供可复现的伪随机数:
# 相同 seed → 完全相同序列, 无任何外部熵源。

_SPLITMIX64_GOLDEN = 0x9E3779B97F4A7C15  # 黄金比例常数

def _splitmix64_next(state: int) -> int:
    """splitmix64: 状态 → 下一个状态 (确定性)"""
    return (state + _SPLITMIX64_GOLDEN) & 0xFFFFFFFFFFFFFFFF

def _splitmix64_value(state: int) -> int:
    """splitmix64: 状态 → 随机值 (确定性)"""
    z = state
    z = ((z ^ (z >> 30)) * 0xBF58476D1CE4E5B9) & 0xFFFFFFFFFFFFFFFF
    z = ((z ^ (z >> 27)) * 0x94D049BB133111EB) & 0xFFFFFFFFFFFFFFFF
    return z ^ (z >> 31)

def splitmix64_int(seed: int, n: int = 0xFFFFFFFFFFFFFFFF) -> int:
    """确定性整数: seed 派生 → 0..n-1 (n 默认 2^64)"""
    if n <= 0:
        raise ValueError("n must be positive")
    v = _splitmix64_value(_splitmix64_next(seed & 0xFFFFFFFFFFFFFFFF))
    return v % n if n != 0xFFFFFFFFFFFFFFFF else v

def splitmix64_sequence(seed: int, count: int) -> list:
    """确定性序列: 从 seed 派生 count 个 64 位随机数"""
    if count < 0 or count > 1_000_000:
        raise ValueError("count out of range")
    state = seed & 0xFFFFFFFFFFFFFFFF
    out = []
    for _ in range(count):
        state = _splitmix64_next(state)
        out.append(_splitmix64_value(state))
    return out

def deterministic_shuffle(seed: int, items: list) -> list:
    """确定性洗牌: 不修改原列表, 返回 Fisher-Yates + splitmix64 的拷贝"""
    result = list(items)
    state = seed & 0xFFFFFFFFFFFFFFFF
    for i in range(len(result) - 1, 0, -1):
        state = _splitmix64_next(state)
        j = _splitmix64_value(state) % (i + 1)
        result[i], result[j] = result[j], result[i]
    return result

def stable_hash(s: str) -> int:
    """稳定 64 位字符串哈希 (FNV-1a 64) — 跨进程可复现"""
...
SAFE_BUILTINS: Dict[str, Any] = {
    # 确定性 PRNG (源自 Cursor splitmix64 逆向)
    "splitmix64_int": splitmix64_int,
    "splitmix64_sequence": splitmix64_sequence,
