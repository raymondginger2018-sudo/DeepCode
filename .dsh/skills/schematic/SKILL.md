...
name: schematic
description: >
  从 git 分支实现逆向生成详细的产品与技术规格文档。适用场景：(1) 分支已上线或进行中需要文档；(2) 需要从产品与架构层面理解分支功能；(3) 接手他人功能分支；(4) 事后编写 PR 描述或设计文档；(5) 用户要求"分析这个分支/从代码写规格/记录该分支做了什么"。产出覆盖问题陈述、产品需求、架构、技术设计、文件清单、测试策略、发布计划与风险的结构化 Markdown 规格。
author: Codex
version: 1.0.0
date: 2026-02-15
